...
try:
    print ("""
              _           _       _                     _
     /\      | |         (_)     | |                   | |
    /  \   __| |_ __ ___  _ _ __ | |     ___   ___ __ _| |_ ___
   / /\ \ / _` | '_ ` _ \| | '_ \| |    / _ \ / __/ _` | __/ _ \

  / ____ \ (_| | | | | | | | | | | |___| (_) | (_| (_| | ||  __/
 /_/    \_\__,_|_| |_| |_|_|_| |_|______\___/ \___\__,_|\__\___|
...
'adm/index.brf','adm.brf','affiliate.brf','adm_auth.brf','memberadmin.brf','administratorlogin.brf']
    ...
    try:
        site = raw_input("Que Web Escanearemos?: ")
        site = site.replace("http://","")
        print ("\tEscaneando sitio web : " + site + "...")
        conn = httplib.HTTPConnection(site)
        conn.connect()
        print "\t[$] Correcto... El Sitio y su servidor estan en linea."
    except (httplib.HTTPResponse, socket.error) as Exit:
        raw_input("\t [!] Ha ocurrido un error, El servidor esta sin coneccion o la URL fue mal escrita")
        exit()
    print "Ingresa el codigo fuente con el que se desarrollo el sitio:"
    print "1 PHP"
    print "2 ASP"
    print "3 CFM"
    print "4 JS"
    print "5 CGI"
    print "6 BRF"
    print "\nteclea 1 and 'Enter' para seleccionar PHP\n"
    code=input("> ")
    ...
    if code==1:
        ...
        for admin in php:
            ...
            if response.status == 200:
                ...
                raw_input("Teclea ENTER para continuar con el escaneo.\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posiblemente pagina de admin  (302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Respuesta de Interes:", response.status)
            connection.close()
        print("\n\nCompletado \n")
        print var1, " paginas de Admin encontradas "
        print var2, " total de paginas encontradas"
        raw_input("[/] Game Over; Teclea ENTER para salir")
    ...
    if code==3:
        ...
        for admin in cfm:
            ...
            if response.status == 200:
                ...
                print "%s %s" % ("\n>>>" + host, "Posible Pagina de Administrador (302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Respuesta de interes:", response.status)
            connection.close()
        print("\n\nCompletado \n")
    ...
    if code==4:
        ...
        for admin in js:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print "%s %s" % ( "\n\n>>>" + host, "Pagina de administrador encontrada!")
                raw_input("teclea ENTER para seguir Escaneando.\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posible Pagina de administrador (302 - Redirect)")
    ...
    if code==5:
        ...
        for admin in cgi:
            ...
            if response.status == 200:
                var1 = var1 + 1
                print "%s %s" % ( "\n\n>>>" + host, "Pagina de administrador encontrada!")
                raw_input("teclea ENTER para continuar escaneando.\n")
            elif response.status == 404:
                var2 = var2
            elif response.status == 302:
                print "%s %s" % ("\n>>>" + host, "Posible pagina de administrador (302 - Redirect)")
            else:
                print "%s %s %s" % (host, " Respuestas de interes:", response.status)
            connection.close()
        print("\n\nCompletado \n")
        print var1, " Paginas de administrador encontradas"
        print var2, " total de paginas escaneadas"
    ...
    if code==6:
        ...
        print("\n\nCompletado \n")
        print var1, " Paginas de administrador encontradas"
        print var2, " total de paginas encotradas"
        raw_input("Game Over; teclea ENTER para salir")
except (httplib.HTTPResponse, socket.error):
    print "\n\t[!] Sesion cancelada; Ocurrio Un error. Verifica tu coneccion a Internet"
